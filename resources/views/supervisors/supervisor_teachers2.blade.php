...
$('#group_message').hide();
        ...
        var url = "{{ URL::to('SMARMANger/dashboard/supervisor/class/lessons2') }}/" + class_id ;
        $.ajax({
            url: url,
            type: "get",
...
$('#group_message').attr('href',"{{ URL('SMARMANger/dashboard/supervisor/send_group_item') }}/"+lesson_id);



        var url = "{{ URL::to('SMARMANger/dashboard/supervisor/lesson/teachers') }}/" + lesson_id;
        $.ajax({
            url: url,
            type: "get",
            contentType: 'application/json',
...
console.log(data);
                ...
                $.each(data, function (key, value) {
                    ...
                    type+=`
                    <a href="{{URL::to('SMARMANger/dashboard/supervisor/send_item/${value.id}/${lesson_id}')}}"  class="btn bg-teal waves-effect" data-toggle="modal" data-id="${value.id}"
                  ><i class="material-icons">forum</i> ارسال عنصر</a>

                  <a href="{{URL::to('SMARMANger/dashboard/supervisor/show_old_item/${value.id}/${lesson_id}')}}"  class="btn bg-teal waves-effect" data-toggle="modal" data-id="${value.id}"
