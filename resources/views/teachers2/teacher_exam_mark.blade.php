            ...
            <div class="container" style="direction: rtl;position: relative;">
                                ...
                                <ul class="nav nav-tabs justify-content-center" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active" data-toggle="tab" href="#profile" role="tab">
                                            <!--i class="now-ui-icons shopping_cart-simple"></i--> امتحانات تقليدية
                                        </a>
                                    </li>

                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#auto_1" role="tab">
                                            <!--i class="now-ui-icons shopping_cart-simple"></i--> امتحانات مؤتمتة
                                        </a>
                                    </li>
                                </ul>
                            </div>
                                ...
                                <div class="tab-content text-center ">
                                    <div class="tab-pane active" id="profile" role="tabpanel">
                                        <div class="container animated bounceInLeft">
                                            <div class="row"
                                                style="justify-content: center;padding-bottom: 20px;padding-right: 66px;">
                                                <div class="col-md-3">
                                                    <!--start add question-->
                                                    ...
                                                    <tbody>
                                                        @foreach ($exam as $item)
                                                            <tr>
                                                                ...
                                                                <td class="py-1" data-label="وقت البداية">
                                                                    {{ $item->start_date }}</td>
                                                                <td class="py-1" data-label="وقت النهاية">
                                                                    {{ $item->end_date }}
                                                                </td>
                                                                <td class="py-1" data-label="الحالة">
                                                                ...
                                                                <td data-label="عرض العلامات"><a
                                                                        href="{{ route('teacher_exam_students', [$room_id, $teacher->id, $lesson_id, $item->id]) }}"
                                                                        class="showstate"><span>عرض العلامات</span></a></td>
                                                                <td data-label="رفع ملف">
                                                                    @if ($item->question_picker == '2')
                                                                        @if ($now < $item->start_date && $now < $item->end_date)
                                                                            <a data-name_quize="{{ $item->name }}"
                                                                                data-type="{{ $item->type }}"
                                                                                data-lesson_id="{{ $lesson_id }}"
                                                                                data-groupe="{{ $item->groupe }}"
                                                                                data-quize="{{ $item->quize }}"
                                                                                data-id="{{ $item->id }}"
                                                                                data-endtime="{{ $item->end_time }}"
                                                                                data-start_time="{{ $item->start_time }}"
                                                                                href="#" data-toggle="modal"
                                                                                data-target="#basicModal" class="bu edit">
                                                                                <i class="fa fa-plus"
                                                                                    style="color: #14315C ; font-size: 20px;"></i>
                                                                                </svg>
                                                                            </a>
                                                                        @elseif($now > $item->start_date && $now < $item->end_date)
                                                                            <a data-name_quize="{{ $item->name }}"
                                                                ...
                                                                <td data-label="اضافة طلاب">
                                                                    @if ($now < $item->start_date && $now < $item->end_date)
                                                                        <a href="#" data-id="{{ $item->id }}"
                                                                            data-room="{{ $room_id }}"
                                                                            class="addstudent showstate" data-toggle="modal"
                                                                            data-target="#basicModal2"><span>اضافة
                                                                                طلاب</span></a>
                                                        ...
                                                        <table class="table table-striped">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td> <a href="{{ route('dashboard.quest_exam1',[ $item->id,$class_id->id,$lesson_id]) }}" class="showstate"><span>مشاهدة</span></a> </td>

                                                                    <td>{{ $item->period }} </td>
                                                                    <td>{{ $item->mark }}</td>
                                                                    <td><a href="{{ route('teacher_exam_students',[$room_id ,$teacher->id,$lesson_id,$item->id]) }}"
                                                                            class="showstate"><span>عرض العلامات</span></a>
                                                                    </td>
                                                                    <td class="py-1">
                                                                        @if($item->question_picker == "2")
                                                                         @if ($now < $item->start_date && $now < $item->end_date)
                                                                         <a class="Btn" href="{{route('exams1_addquestion',[$item->id,$item->room->id,$class_id->id,$lesson_id]) }}"
                                                                            style="margin: auto;">
                                                                            <div class="sign">+</div>
                                                                        </a>
                                                                           @else
                                                                    ...
                                                                    <td>
                                                                         @if ($now < $item->start_date && $now < $item->end_date)
                                                                        <a href="#" class="addstudent1 showstate"
                                                                            data-toggle="modal"
                ...
                <div class="modal fade" id="basicModal" tabindex="-1" role="dialog" aria-labelledby="basicModal"
                    ...
                    <div class="modal-dialog">
                        <form action="{{ route('dashboard.exam.update123') }}" method="post"
                                ...
                                <div class="modal-body">
                                    <div class="">
                                        ...
                                        <label for="file-input" class="drop-container">
                                            ...
                                            <input type="file" accept="image/*" name="quize" required=""
                                                id="file-input">
                                        </label>
                                    </div>
                                </div>
                                <div class="modal-footer" style="justify-content: center;">
                                    <button type="button" class="addquestion" data-dismiss="modal"
                                        style="width: 80px;
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer"style="justify-content: center;">
                                    <button type="button" class="addquestion" data-dismiss="modal"
                                        style="width: 80px;
                                                height: 38px;">اغلاق</button>
                                    <button id="save_student" type="submit" class="addquestion"
                                        style="width: 80px;
                                                height: 38px;">حفظ</button>
                                </div>
                            </div>
...
@section('js')
    ...
    <script>
        $(".addstudent").on("click", function(e) {
            ...
            $('.exam').val(exam);
            $('.room_id').val(room);
            var url = "{{ URL::to('SMARMANger/dashboard/teacher/studentselect') }}/" + exam + "/" + room;
            $.ajax({
                url: url,
        ...
        $(".edit").on("click", function(e) {
            ...
            if (type == '5') {
                ...
                var url = "{{ URL::to('SMARMANger/dashboard/teacher/detexam') }}";
                $.ajax({
                    url: url,
                    data: data,
                    type: "get",
