...
@section('content')
    ...
    padding-top: 11px;">
            ...
            <li class="li"><a href="{{ route('dashboard.teacher.exams',[$exam->class_id,$lecture_id,$room_id]) }}">الاختبارات</a></li>
        ...
        <div class="content-wrapper pb-0">
            ...
            <div class="container2" style="padding-bottom: 150px;">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form">
                                ...
                                <input class="input" ame="search"   id="search" placeholder="ابحث عن السؤال" type="search">
                        ...
                        <div class="col-md-4" style="padding-top: 24px;">
                            ...
                            <!--    </div>-->
                            <!--</div>-->
                            <p style="padding-left: 9px;">{{ $exam->success_mark }} / <span id="mark22"></span></p>
                        </div>
                ...
                <form action="{{ route('dashboard.exams.myquestions') }}" class="limiter" method="post" autocomplete="off">
                    @csrf
                    ...
                    <input type="hidden" name= "selected_ques2" data-selected_ques="{{ $exam->selected_ques }}"
                        id="selected_questions">

                    <input type="hidden" value="{{ $exam->id }}" id="exam" name="exam_id">
                    <input type="hidden" value="{{ $exam->success_mark }}" id="success_mark" name="success_mark">
                    <div class="r"></div>
                    <div class="container">
                        <svg  viewBox="0 0 0 0" style="position: absolute; z-index: -1; opacity: 0;width:30px">
                            <defs>
                              <linearGradient id="boxGradient" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="25" y2="25">
                                <stop offset="0%"   stop-color="#4382e0"/>
                                <stop offset="100%" stop-color="#4382e0"/>
                              </linearGradient>
                        ...
                        <div class="todo-list lecq ">
                            ...
                            @if ($item1 == $item->id)
                                @php
                                    $i23 = $item->id;
                                @endphp
                                <input type="text" hidden value="{{ $item->id }}" class="y "
                                    id="cb{{ $item->id }}">
                            @endif
                            @endforeach
                            ...
                            <label class="todo"  for="cb{{ $item->id }}">
                                ...
                                <div class="todo__text">{{ $item->question_form }}
                                    ({{ $item->mark }})</div>
                            </label>
...
<script>
    ...
    $(document).on('keyup', '#search', function () {
        var lect=$('#search').val();
                ...
                var url = "{{ URL::to('SMARMANger/dashboard/teacher/search_test') }}";
            $.ajax({
                url: url,
                data : data,
                type: "get",
             ...
             let checked ;
                    ...
                    $.each(data, function (key, value) {
                        ...
                        $('.lecq').append(`  <li>
                        <input type="checkbox"  class="x" ${checked} value="${ value.id }" id="cb${ value.id }" data-mark="${ value.mark }" name="selected_ques[]" >
    ...
    $(document).on('change', '.choice', function () {
                ...
                $('.lecq').empty();
            var url = "{{ URL::to('ROYAMANger/dashboard/teacher/lecquestion') }}/" + lect +"/"+exam;
            $.ajax({
                url: url,
                type: "get",
                contentType: 'application/json',
       ...
       $(document).on('click', '.x55', function () {
           var x1=0;
            var x3=localStorage.getItem('x');
             if ($(this).is(':checked')) {
