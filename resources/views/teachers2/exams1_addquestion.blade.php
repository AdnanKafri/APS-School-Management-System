    ...
    padding-top: 11px;">
        ...
        <div class="content-wrapper pb-0">
            ...
            <div class="container2" style="padding-bottom: 150px;">
                ...
                <form action="{{ route('dashboard.exams.myquestions1') }}" class="limiter" method="post" autocomplete="off">
                    @csrf
...
<script>
          ...
          $('#mark22').text(x1);
           localStorage.setItem('x', x1);
                ...
                console.log(Object.keys(selected_ques).length);
                        selected_ques[Object.keys(selected_ques).length] = $(this).val()
                    exam_questions[exam_id] = selected_ques;
                    localStorage.setItem('exam_questions', JSON.stringify(exam_questions));
                    console.log(selected_ques);
                     $('.r').empty();
    ...
    $(document).on('keyup', '#search', function () {
        var lect=$('#search').val();
                ...
                var url = "{{ URL::to('SMARMANger/dashboard/teacher/search_eaxm') }}";
            $.ajax({
                url: url,
                data : data,
                type: "get",
             ...
             let checked ;
                    ...
                    //     $('.lecq').append(`
                    //     <label class="todo"  for="cb${ value.id }"">
                    //             <input value="{{ $item->id }}"
                    //             data-mark="${ value.mark }" ${checked} value="${ value.id }"  class="todo__state x  s123" id="cb${ value.id }" name="selected_ques[] type="checkbox" />
                    ...
                    //             </svg>
                    //             <div class="todo__text">${ value.question_form } (${ value.mark })</div>
                    //         </label>
                    //     <!-- <li>
    ...
    $(document).on('change', '.choice', function () {
                ...
                var exam=$('#exam').val();
                $('.lecq').empty();
            var url = "{{ URL::to('SMARMANger/dashboard/teacher/lecquestion1') }}/" + lect +"/"+exam;
            $.ajax({
                url: url,
                type: "get",
                contentType: 'application/json',
