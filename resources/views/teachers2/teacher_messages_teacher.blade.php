...
@endif
    <div class="main-panel">
        ...
        <ul class="breadcrumbs" style="padding-bottom: 7px;
            ...
            <li class="li"><a href="#">رسائل الطلاب</a></li>
        ...
        <div class="content-wrapper pb-0">
            ...
            <div class="row" style="float: right;">
                <div class="col-md-4">
                    <a href="{{ route('dashboard.teacher_messages') }}" class="live" style="text-decoration: none;">
                       ارسال رسالة للكل
                    </a>
                </div>
            </div>
            ...
            <div class="container mcontainer" style="padding-top: 100px; padding-bottom:100px">
                <div class="panel messages-panel" style="border: 1px solid #4da2eb;">
                                    ...
                                    <ul class="list-unstyled contacts">
                                        @foreach ($student as $item)
                                            @if ($item->message_count > 0)
                                                <li  class="message1" aria-selected="true" data-toggle="tab" data-target="#std{{ $item->id }}" data-id="{{ $item->id }}" class="act_student">
                                                    <div class="message-count number">{{ $item->message_count }}</div>
                                                    @if ($item->image)
                                        ...
                                        @foreach ($student as $item)
                                            @if ($item->message_count == 0)
                                                <li data-toggle="tab" data-target="#std{{ $item->id }}" data-id="{{ $item->id }}" class="act_student">
                                                    <div class="message-count">{{ $item->message_count }}</div>
                                                    @if ($item->image)
                                                        <img src="{{ asset('storage/' . $item->image) }}"
                                                            class="img-circle medium-image" />
                                                    @else
                    ...
                    <div class="tab-content">
                        @foreach ($student as $item)
                            <div class="tab-pane message-body " id="std{{ $item->id }}" style="">
                                <div class="message-chat">

                                    <div class="chat-body " id="{{ $item->id }}">
                                     <div class="chat-body1">

                                        @foreach ($item->message1 as $item1)
                                            @if ($item1->type == '1' && $item1->student_id == $item->id)
                                                <div class="message info">
                                                    ...
                                                    <h5><i class="fa fa-clock-o"></i> {{ $item1->created_at }} </h5>
                                                </div>
                                                <hr>
                                    ...
                                    <div class="flex-grow-0 py-3 px-4 border-top">
                                        <div class="input-group">
                                            ...
                                            <input type="hidden" class="teacher_id" name="teacher_id" value="{{ $teacher->id }}">
                                           <input type="hidden" class="student_id" name="student_id" value="{{ $item->id }}">
...
<script>
    $(".message1").on("click", function (e) {
        ...
        var element =$(this) ;
         ...
         var url = "{{ URL::to('SMARMANger/dashboard/teacher/show_message') }}/" +id ;
                  $.ajax({
                      url: url,
 ...
 $('input').keypress(function(event) {
    if (event.which == 13) {
        ...
        student_id=$(this).parent().find(`.student_id`).val();
             var url = "{{ route('dashboard.send_message') }}";
            $.ajax({
                url: url,
                data: {
                    ...
                    $.each(data, function(index, value) {
                        ...
                        if (value.message_count > 0) {
                            $('.list-unstyled').prepend(`
                                <li data-toggle="tab" data-target="#std${ value.id }" data-id="${ value.id }" class="act_student message1">
                                    <div class="message-count number">${ value.message_count }</div>
                                      ${image}
