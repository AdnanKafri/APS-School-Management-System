...
<div class="main-panel" style="background: #f8f9fb;">
    ...
    <li class="li"><a href="{{ route('dashboard.coordinator.questions',[$questions->class_id,$room_id,$questions->Lecture->id,$questions->lesson_id ]) }}">الاسئلة</a></li>
    <li class="li"><a href="#">تعديل سؤال</a></li>
    </ul>
        @if (session()->has('update'))
        <script>
             window.onload = function () {
        notif({
            msg: "تم تعديل  السؤال بنجاح ",
            type: "success"
        })
    }
        </script>
        ...
        <div class="container2" style="padding-bottom: 150px;">
            <div class="heading">تعديل سؤال</div>
                    {{-- <a href="{{ route('dashboard.teacher.questions',[$questions->class_id,$room_id,$questions->Lecture->id,$questions->lesson_id ]) }}">  <button
                        class="btn btn-primary rounded-0 py-2 px-4" style="float: left;">عودة</button></a>
                  <h3 style="color: #094e89;">تفاصيل السؤال </h3> --}}
                        <form action="{{ route('coordinator.question.update',$id) }}" method="post"
                            class="mb-5" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" class="form-control" name="class_id"  value="{{ $questions->class_id }}">

                            <div class="row">
                                <div class="col-md-12 form-group mb-3">
                                    <label for="budget" class="col-form-label">الدرس  </label>
                                    <select class="custom-select" name="lecture_id">


                                        <option value="{{ $questions->Lecture->id }}" name="class_id"


                                             >{{ $questions->Lecture->name}}</option>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 form-group mb-3">
                                    ...
                                    <input type="hidden" class="form-control" name="ques_type" id="ques_type" value="{{ $questions->ques_type }}">
									<input type="text" class="form-control" name="ques_typee"  value="{{ $questions->ques_type == 2 ? 'تقليدي / نصي ' : ($questions->ques_type == 3 ? 'تقليدي / ملف' : 'متعدد الخيارات')}}" disabled>

                                    <br>
                                    <br>
                                    @if($questions->ques_type == 2)
                                     <input type="hidden"  class="myselection" value="2">


                                    <div id="show2" class="myDiv" style="display: block">
                                        <div class="row">
                                            <div class="col-md-12 form-group mb-3">
                                                <label for="message" class="col-form-label">ادخل نص السؤال </label>
                                                <textarea class="form-control" name="question_form"  cols="30" rows="3">{{ $questions->question_form }}</textarea>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12 form-group mb-3">
                                                <label for="message" class="col-form-label">ادخل الجواب </label>
                                                <textarea class="form-control input12"  id="question_form22"  name="answer" cols="10" rows="3">{{ $questions->answer }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    @elseif($questions->ques_type == 3)
                                    <input type="hidden"  class="myselection" value="3">

                                    <div id="show3" class="myDiv" style="display: block">
                                        <div class="row">
                                            <div class="col-md-12 form-group mb-3">
                                                <label for="message" class="col-form-label">ادخل نص السؤال </label>
                                                <textarea class="form-control" name="question_form"  cols="30" rows="3">{{ $questions->question_form }}</textarea>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 form-group mb-3">
                                                <label for="message" class="col-form-label"> أدخل جواب جديد  كملف </label>
                                                <input type="file" class="form-control"  id="question_form33"  name="answer" >
                                            </div>
                                            <br>
                                            <br>
                                            <div class="col-md-12 form-group mb-5 mt-4">
                                                {{-- <label for="message" class="col-form-label text-large text-dark">    الإجابة السابقة </label> --}}
                                                <a  href=" {{ asset('storage/'.$questions->answer) }}" class=" btn btn-sm btn-secondary" style="font-size: 14px;font-weight: 700" target="_blank" title="عرض المحتوى"> عرض الإجابة السابقة</a>
                                            </div>
                                        </div>
                                    </div>
                                    @endif
...
selected
                ...
                @endforeach
                            ...
                            @if($questions->ques_type == 1)
                    <input type="hidden"  class="myselection" value="1">

                            <div id="show1" class="myDiv" style="display: block">
                                <!-- start my div-->
                                <div class="row">
                                ...
                                @php
                                $options = json_decode($questions->option->myOptions,true) ;
                                ...
                                <div id="add">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 form-group mb-3" style="background-color:#fff;">
                                            <label for="" class="col-form-label"> الخيار التالي*</label>
                                            <input type="text" name="option[]" readonly class="form-control input input12 "
                                                placeholder="ادخل الخيار الأول " value="{{ $value }}">
                                                 <label style="font-size: small;color: red;">لايمكن التعديل لان السؤال مستخدم في امتحان سابقاً</label>
                                        </div>
                                    </div>
                                    @else
                                    <div class="row">
                                    ...
                                    <script>
                                        $(document).on('click', '#btn', function () {
                                            $('#add').append(
                                                '<div class="row" id="register"><div class="col-md-12 form-group mb-3"><label for="" class="col-form-label"> </label> <input type="text" class="form-control input input12" name="option[]" placeholder="ادخل الخيار التالي "></div></div>'
                                            );
    ...
    inputt.splice(index, 1);
                            ...
                            <div class="row">
                                <div class="col-md-12 form-group mb-3">
                                    ...
                                    <input type="number" min="0"   class="form-control" step="0.01"  data-mark="{{ $questions->mark }}" value="{{ $questions->mark }}" name="mark" id="mark"
                            ...
                            <div class="row">
                                <div class="col-md-12 form-group mb-3">
                                    ...
                                    <textarea class="form-control" name="note" value="{{ $questions->note }}" cols="30" rows="3"
                                        placeholder="ادخل الملاحظات "> {{ $questions->note }}</textarea>
                                </div>
                            </div>
                            <br>
                            <div class="row">
