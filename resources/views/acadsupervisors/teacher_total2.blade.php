...
<section class="hero-wrap hero-wrap-2" style="background-image: url( {{  asset('teachers/ppp.jpg') }});">
 ...
 <nav class="breadcrumbs">
    ...
    <a  href="{{ route('dashboard.acadsupervisor_subject',$room->id ) }}" class="breadcrumbs__item ">{{ $class_id->name }} / {{$room->name}}   </a>
...
<!-- marks of subjects -->
<a  href="{{ route('StudentsRoomLessontotal_pdf12',[$room_id,$teacher->id,$lesson_id]) }}"   class="btn" style=" background: linear-gradient(to right top, #2c71ad 20%, #84a7c4);color :white;" target="_blank">pdf تنزيل </a>
<a  href="{{ route('StudentsRoomLessontotal_excel12',[$room_id,$teacher->id,$lesson_id]) }}"   class="btn" style=" background: linear-gradient(to right top, #2c71ad 20%, #84a7c4);color :white;" target="_blank">excel تنزيل </a>
            ...
            <input type="hidden" name="lesson_id" value="{{ $lesson_id }}">
             ...
             <td>
              ...
              <input class="number" style="height: 50px; width:60px;border: 1px solid #e9d8db;"  value="{{ json_decode($item2->mark,true)[$lesson_id]['exam'] }}" name="exam" onfocus="this.placeholder = ''" onblur="this.placeholder = ''" class="common-input mb-20 form-control"  type="number" min="0" data-max="{{$lesson->max_mark*0.4}}"  max="{{$lesson->max_mark*0.4}}">
                ...
                @else
                 <td style="color: red;
               text-decoration: underline;"> {{ json_decode($item2->result1,true)[$lesson_id]['term1_result'] }} </td>
                @endif
                   @if(json_decode($item2->result1,true)[$lesson_id]['term1_result'] >$lesson->min_mark)
                <td> {{arabic_w2e( json_decode($item2->result1,true)[$lesson_id]['term1_result'] )}} </td>
                @else
        ...
        <thead>
            <tr>
              <th rowspan="2"   style="text-align: center;" >الرقم </th>
                ...
                <th rowspan="2" colspan="1" style="text-align: center;"> مجموع المحصلتين <span style="color:white;"> &#247</span> 2 </th>
