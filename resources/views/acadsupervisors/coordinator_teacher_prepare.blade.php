...
<body  style="background-image: url( {{  asset('teachers/images/new2.jpg') }});
  ...
  <nav class="breadcrumbs" style="float: left;">
    ...
    <a   href="{{ route('dashboard.acadsupervisor_teacher_lesson',[$room->id,$teacher->id,$lesson->id]) }}" class="breadcrumbs__item ">الدروس  </a>
   
     <a   href="{{ route('dashboard.acadsupervisor') }}" class="breadcrumbs__item ">الواجهة الرئيسية 
</a>
</nav>
    <div class="content" style="direction: rtl; text-align: right; padding-bottom: 16px;">
            ...
            <div class="row align-items-stretch no-gutters contact-wrap" style="border-radius: 15px;">
                <div class="col-md-12 col-xs-12">
                    ...
                    <div class="form h-100"  style="border-radius: 15px;">
                        ...
                        <div class="search">
                            <input class="search__input" id="search" placeholder="البحث .." />
                              <input id="teacher_id"  hidden   value="{{$teacher->id}}" />
                            
                            <button  id="gpdf" class="fa fa-search search__button"></button>
                        </div>
                        <div class="select"   style=" border-radius: 5px; float: left;width: 200px;">
                            <select class="download">
                                <option value="1" > تنزيل دفتر التحضير</option>

                                <option value="3"  >تنزيل صفحة </option>
                                <option value="4">تنزيل كامل الدفتر </option>
                        ...
                        <h3 style="color: #094e89;float: left;">العام الدراسي {{ $year->name }}</h3>
                        <h3 style="color: #094e89;">دفتر التحضير </h3>
                        <div class="table_data"  id="divIdToPrint">
                            @include('acadsupervisors.load')
                        </div>



                     <input style="width: 200px; text-align: center;margin-top: -20px;"  hidden id="lesson_name"
                            class="common-input mb-20 form-control" disabled     value="{{ $lesson_id->name }}" type="text">
                        <input style="width: 200px; text-align: center;margin-top: -20px;" id="lesson_id"
                            class="common-input mb-20 form-control" hidden value="{{ $lesson_id->id }}" type="text">
                        <input hidden value="{{ $term->id }}" name="term_id" id="trem_id" type="text">&nbsp;&nbsp;&nbsp;

                        <input style="width: 100px; text-align: center;margin-top: -20px;" hidden id="class_name"
                            value="{{ $class_id->name }}" disabled class="common-input mb-20 form-control"
                            type="text">&nbsp;&nbsp;&nbsp;

                        <input style="width: 100px; text-align: center;margin-top: -20px;" id="class_id" name="class_id"
                            hidden value="{{ $class_id->id }}" disabled class="common-input mb-20 form-control"
                            type="text">
     ...
     var teacher_id=$('#teacher_id').val();
            ...
            var term=$('#trem_id1').val();
                    ...
                    var data={
                        ...
                        $('.table1 tbody').empty();
                    var url = "{{ URL::to('SMARMANger/dashboard/acadsupervisor/searchlect1') }}";
                $.ajax({
                    url: url,
                    data : data,
                    type: "get",
