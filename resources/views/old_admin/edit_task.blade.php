...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-12">
                <form action="{{ route('admin.teacher.update_set_task') }}" method="post"  enctype="multipart/form-data" >
                        ...
                        <div class="form-group class_lessons" id="" style="text-align:right">
                            ...
                            <select name="lesson_id[]" id="" class="form-control lessons two"
                                ...
                                <div class="form-group class_rooms" id="">
                                    ...
                                    <select name="room_id[{{ $item->id }}][]" id="" class="form-control rooms dep"
                                        style="min-height: 36px;direction:rtl" multiple required>

                                        @if (App\Room::where('class_id',$item->class_id)->where('year_id',$year->id)->count() !=0)

                                    <option value="0">كافة الشعب</option>

                                        @endif


                                        @foreach (App\Room::where('class_id',$item->class_id)->where('year_id',$year->id)->get() as $item2)

                                        @php
                                        $a=[];

                                        @endphp
                                        @foreach ($item->rooms as $item_room)
                                        @if ($item2->id == $item_room->id)
                                        <option value="{{$item2->id}}" selected>{{ $item2->name }}
...
$(document).on('change', '.classes', function () {
            ...
            var url = "{{ URL::to('SMARMANger/admin/classes/teacher_lessons') }}/" + class_id ;
            $(this).addClass('x');
