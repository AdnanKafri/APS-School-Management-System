...
<nav class="breadcrumbs">
    <a  class="breadcrumbs__item  is-active">برامج جميع المعلمين</a>
...
@endif
    ...
    <br><br>
    <input type="button" id="button" class="btn btn-success" value="تنزيل" style="font-size: 40px">
    <br>

    <div class="print_all" >
    @foreach ($schedules as $key_teacher => $schedule)

    <div class="row long_long" id="photo_do" >
        <h1  class="title text-center w-100 m-5" style="text-align: center"> برنامج المدرس {{ $teacher[$key_teacher]->first_name }} {{ $teacher[$key_teacher]->last_name }}  </h1>
        <br><br><br><br>
       <table class="table table-bordered table-responsive" id="simple_table"
                    ...
                    <tr>
                        <th scope="row">{{ $day->name }}</th>
                            ...
                            @if ($day->id == $lesson_time->day_id )
                                @php
                                    $background = '';
                                    if( $today == $day->id - 1 && $lesson_time->attendance == false)
                             ...
                             <td >
                                <a class="btn {{ $background }} btn-sm add_time "  title="الدخول إلى الحصة">
                                    ...
                                    <p class="teacher_name-schedule{{  $day->id .''. $lesson_time->lecture_time->id }}" style="margin:0;font-size:16px"> {{ $lesson_time->room->classes->name." / ".$lesson_time->room->name }} </p>
                                    <p  style="margin:0;font-size:16px"> {{ $lesson_time->lecture_time->start_time." - ".$lesson_time->lecture_time->end_time }} </p>
                                    <p  style="margin:0;font-size:16px"> {{ $lesson_time->lecture_time->name }} </p>
                                </a>
                            </td>
                            @endif
                        @endforeach
  ...
  <br>
    ...
    <script>
        $(document).on("click","#button",function () {
            ...
            $('.long_long').css('min-height','1530px');
            $(".print_all").printArea(options);
            $('.long_long').css('min-height','100%');
