...
<nav class="breadcrumbs">
    <a  class="breadcrumbs__item is-active">  اقسام البناء </a>
...
<div class="modal fade editModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{route('construction_departments_update')}}"  enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="create_section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none; padding-right: 17px;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('construction_departments_store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
...
<div class="card" style="margin: 30px">
    <div class="card-body">
        <div class="card-title">
            <h1 style="text-align: center;color: #001586">جدول اقسام   البناء</h1>
        </div>
          @can('add_construction_departments')
        <button type="button" class="btn mb-1 btn-success" data-toggle="modal" data-target="#create_section" style="font-size: 25px;font-weight: 600;float: right;background: #6ABAA3;border-color: #6ABAA3">إنشاء قسم </button>
       @endcan
        <div class="">
            <table class="table align-items-center " id="table_xx">
                ...
                <tbody class="list" id="mydiv">
                 @foreach ($construction_departments as $item)
                 <tr>
                 <td>{{$item->name}}</td>
                 <td>
                     @can('edit_construction_departments')
                    ...
                    @endcan
                    @can('delete_construction_departments')
                    <a href=".deletelessonModal" class="btn btn-danger delete"
                    data-id="{{ $item->id }}"  data-toggle="modal" >
                        حذف
                    </a>
                    @endcan
                    @can('department_details')
                    <a href="{{ route('department_details',$item->id) }}" class="btn btn-danger delete">
                         تفاصيل القسم 
                    </a>
                    @endcan
...
<div class="modal fade deletelessonModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form_delete" action="{{route('construction_departments_delete')}}" method="POST"  autocomplete="off">
