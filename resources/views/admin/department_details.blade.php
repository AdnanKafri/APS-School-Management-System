...
<nav class="breadcrumbs">
    <a  class="breadcrumbs__item is-active">    مكونات الاقسام  </a>
    <a href="{{ route('construction_departments') }}" class="breadcrumbs__item ">  اقسام البناء  </a>
...
<div class="modal fade editModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{route('department_details_update')}}"  enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label> الكمية </label>
                        <input type="number" name="quantity"  id="quantity" class="form-control a" style="direction:rtl"
                            value=""
                            placeholder=" الكمية  " required>
                    </div>
                    <div class="form-group">
                        <label>    الوصف  </label>
                        <textarea  name="description" id="description" class="form-control a" style="direction:rtl"
                           required></textarea>
                    </div>
                     <div class="form-group">
                        <label>  اضافة ملف </label>
...
<div class="modal fade" id="create_section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none; padding-right: 17px;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('department_details_store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span>
                    </button>

                    <h2 class="modal-title" id="exampleModalLabel">اضافة  مكون  </h2>
                    <input type="hidden" name="construction_department_id"  value="{{$construction_department_id}}">
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label>اسم  المكون </label>
                    ...
                    <div class="form-group">
                        <label> الكمية </label>
                        <input type="number" name="quantity" class="form-control a" style="direction:rtl"
                            value=""
                            placeholder=" الكمية  " required>
                    </div>
                    <div class="form-group">
                        <label>    الوصف  </label>
                        <textarea  name="description" class="form-control a" style="direction:rtl"
                           required></textarea>
                    </div>
                       <div class="form-group">
                        <label>  اضافة ملف </label>
...
<div class="card" style="margin: 30px">
    <div class="card-body">
        <div class="card-title">
            <h1 style="text-align: center;color: #001586">جدول    مكونات الاقسام</h1>
        </div>
        @can('add_department_details')
        <button type="button" class="btn mb-1 btn-success" data-toggle="modal" data-target="#create_section" style="font-size: 25px;font-weight: 600;float: right;background: #6ABAA3;border-color: #6ABAA3">إنشاء  مكون  </button>
       @endcan
        <div class="">
            <table class="table align-items-center " id="table_xx">
                ...
                <tr>
                    ...
                    <th scope="col" class="sort" data-sort="budget"> النوع  </th>
                    <th scope="col" class="sort" data-sort="budget"> الكمية  </th>
                    <th scope="col" class="sort" data-sort="budget"> الوصف   </th>
                ...
                <tbody class="list" id="mydiv">
                 ...
                 <tr>
                 <td>{{$item->name}}</td>
                 <td>{{$item->construction_department->name}}</td>
                 <td>{{$item->quantity}}</td>
                 <td>{{$item->description}}</td>
                 <td>@if($item->file)
                     <a href="{{asset("storage/")}}/{{$item->file}}" target="_blank" style="color: #b4b3e6">
...
<div class="modal fade deletelessonModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form_delete" action="{{route('department_details_delete')}}" method="POST"  autocomplete="off">
                ...
                <div class="modal-body">
                    ...
                    <div class="py-3 text-center">
                        ...
                        <h4 class="heading mt-4">حذف   مكون</h4>
