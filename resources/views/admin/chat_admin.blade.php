...
{{-- <div class="row ml-5 mt-5" style="color: white ;display: flex;j">
    <div class="col-12 col-lg-2" >
        <a class="btn btn-success" style="font-size:20px" href="{{ route("all_chat") }}" > <i class="fa fa-arrow-circle-left">  </i> محادثات الجميع </a>
    </div>
  </div> --}}
          ...
          <div class="card chat-app" style="height: calc(100vh - 100px);">
              <div id="plist" class="people-list" style="height: 100%">
                  ...
                  <ul class="list-unstyled chat-list mt-2 mb-0" style="overflow-y: scroll;height: 90%">
                    ...
                    @endphp
                        <li class="clearfix user_list user_list{{ $item->id }}" data-id="{{ $item->id }}">
                            <div class="about p-0" style="width:100%">
                                <img src="{{ asset('website/user.png') }}" alt="avatar">
                                <div class="name name_user" style="color:{{$color}}">
                                    @if($item->type == "1")
                                            {{ $item->teacher != null ? $item->teacher->first_name." ".$item->teacher->last_name : ""  }}
                                    @elseif($item->type == "2")
                                            {{ $item->name }}
                                    @elseif($item->type == "3")
                                            {{ $item->supervisor != null ? $item->supervisor->first_name." ".$item->supervisor->last_name : "" }}
                                    @elseif($item->type == "4")
                                            {{ $item->coordinator != null ? $item->coordinator->first_name." ".$item->coordinator->last_name : "" }}
                                    @elseif($item->type == "5")
                                            {{ $item->acadsupervisor != null ? $item->acadsupervisor->first_name." ".$item->acadsupervisor->last_name : "" }}
                                    @endif
                                </div>
                                <div class="status"> @if(sizeof($item->chats) > 0) <span class="count_message" > {{ sizeof($item->chats) }} </span> @endif </div>
                            </div>
                        </li>
                    @endforeach --}}
                  </ul>
