...
<nav class="breadcrumbs">
    <a  class="breadcrumbs__item is-active">قسم مواد الصف</a>
...
@section('content')
    ...
    <div class="card" style="direction:rtl;text-align:right;margin: 20px">
            ...
            <div class="card-header border-0">
              <h1 class="mb-0" style="text-align: center">جدول المواد /  {{ $class->name }}</h1>
               @can('create_lesson')
                    ...
                    data-id=""><i class="material-icons" data-toggle="tooltip">إنشاء مادة الديانة</i>
                    </a>
                    @endcan
                    @can('base_subject')
                     <a href="{{ route('admin.base_subjects',$class->id) }}" class=" btn btn-info" id="two"
                    style="background: #07d49a !important;border-color: #6ABAA3;float:left;"
                    data-id=""><i class="material-icons" > الأقسام الأساسية</i></a>
                 @endcan
...
<div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
               ...
               <tr  id="lesson_{{$item->id}}">
                    ...
                    <td class="budget">
                        ...
                        <div class="avatar-group">
                              <a href="#"  class="avatar avatar-sm rounded-circle" data-toggle="tooltip" data-original-title="Ryan Tompson">
                               <img  style="width: 100px;" alt="Image placeholder" src="{{ asset('storage/'.$item->img) }}">
                         </a>
                    ...
                    <td>
                        @can('update_lesson')
                        <a href=".editlessonModal" class="edit"   data-religion="{{ $item->religion }}"  data-lang="{{ $item->lang }}"  data-is_english="{{ $item->is_english }}"
                        data-type="{{ $item->type }}" data-name_en="{{ $item->name_en }}" data-name="{{ $item->name }}"
                        data-class1="{{ $item->classes->id }}" data-id="{{ $item->id }}" data-base_subject_id = "{{ $item->base_subject_id }}"
                        data-mark_base_subject_id = "{{ $item->mark_base_subject_id }}"
                        data-max_mark= "{{ $item->max_mark }}" data-min_mark= "{{ $item->min_mark }}"
                        data-certificate_order = "{{ $item->certificate_order }}"
                        data-is_addable = "{{ $item->is_addable }}"
                        data-is_behavior = "{{ $item->is_behavior }}"
                        data-is_project = "{{ $item->is_project }}"
                        data-first_total = "{{ $item->first_total }}"
                        data-is_neutral = "{{ $item->is_neutral }}"
                         data-not_affect_and_collect = "{{ $item->not_affect_and_collect }}"
                       ...
                       @can('add_lesson_book')
                        <a data-id="{{ $item->id }}" data-books="{{ $item->books }}" class="bookmodal2"  data-toggle="modal" data-target=".bookmodal" title="الكتب">
                            <i style="font-size: 30px; color: #008CC4" class="fa fa-plus"></i>
                        </a>
                         @endcan
                          ...
                          @can('delete_lesson')
                         <a href=".deletelessonModal" class="delete"   data-name_en="{{ $item->name_en }}" data-name="{{ $item->name }}" data-class1="{{ $item->classes->id }}" data-id="{{ $item->id }}"  data-toggle="modal" >
                            <i class="fa fa-trash" style="font-size: 30px;color: #af686e"></i>
                        </a>
                        @endcan
                        @can('show_lesson_book')
                        <br>
                           @if ($item->books != null)
                        @foreach (json_decode($item->books) as $book)
                                <a target="_blank" @if ($book->type == "link")
                                href="{{ $book->value }}"
                            @else
                                href="{{ asset('storage/'.$book->value) }}"
                            @endif   data-books="" class="" title="الكتب">
                                    <i style="font-size: 20px; color: #008CC4" class="fa fa-book"></i>
                                </a>
                         @endforeach
                         @endif
                         @endcan
  ...
  <div class="modal-dialog modal-danger modal-dialog-centered modal-" role="document">
      <div class="modal-content bg-gradient-danger">
          ...
          <div class="modal-footer" style="justify-content: right;">
              ...
              <a class="btn btn-link text-white ml-auto" data-dismiss="modal">Close</a>
                        ...
                        <div class="modal-content">
                            <form id="" action="{{ route('lesson_store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>القسم الذي تتبع له  </label>

                                        <select name="base_subject_id" id="base_subject" class="form-control dep"
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        ...
                                        <select name="class_id" id="classes" class="form-control dep"
                                         ...
                                         <div class="form-group" style="text-align:right">
                                        <label for="" style="text-align:right" >صورة المادة </label><br>
                                        <input type="file" name="image" id="" class="form-control a" width="100%">
                                </div>
                                    <br>
                                    <div class="form-group" style="text-align:right">
                                        <p Style="font-weight: bold;color:#0069d9">تفصيلات خاصة بالجلاء المدرسي</p>
                                    </div>
                                    <div class="form-group" style="text-align:right">
                                        <label>القسم الذي تتبع له في الجلاء المدرسي </label>

                                        <select name="mark_base_subject_id" id="" class="form-control dep"
                                            style="min-height: 36px;direction: rtl" >
                                            <option value="">اختر القسم في الجلاء </option>
                                    ...
                                    @if($class->stage_id < 3 )
                                    <div class="form-group" style="text-align:right">
                                        <label> هل المادة قابلة للجمع    </label>
                                         <select name="is_addable" id="" class="form-control "
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل المادة قابلة للإضافة  </option>

                                                <option value="1">نعم</option>
                                                <option value="0"> لا مثال */ اللغة العربية */ </option>
                                    ...
                                    @endif
                                        ...
                                        <div class="form-group" style="text-align:right">
                                        <label> هل المادة  سلوك      </label>
                                         <select name="is_behavior" id="" class="form-control"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل المادة  سلوك    </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>ترتيب الظهور في الجلاء المدرسي </label>

                                            <input type="number" name="certificate_order" class="form-control "
                                                value="" min="0" max="16"
                                                placeholder="  أدخل الترتيب" maxlength="2" required>
                                    </div>
                                    <div class="form-group" style="text-align:right">
                                        <label> هل المادة مادة مشروع    </label>
                                         <select name="is_project" id="" class="form-control "
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden> حدد  هل المادة مادة مشروع     </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>    تدخل ضمن المجموع الأول    </label>
                                         <select name="first_total" id="" class="form-control "
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل تدخل المادة  في المجموع الأول   </option>

                                                <option value="1">نعم</option>
                                                <option value="0"> لا مثال */  التربية المهنية */ </option>

                                        </select>
                                    </div>
                                    <div class="form-group" style="text-align:right">
                                        <label>    أثر المادة على النجاح والرسوب       </label>
                                         <select name="is_neutral" id="" class="form-control"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل تؤثر المادة  على النجاح    </option>

                                            <option value="1">  تؤثر وتعتبر مادة مرسبة  </option>
                                            <option value="2">  تؤثر ولا تعتبر لوحدها مادة مرسبة  </option>
                                              <option value="4">
                                                 تدخل في المجموع لكن ليست مادة مرسبة

                                                 </option>
                                            <option value="3">مادة حيادية لاتؤثر ولا تحسب بالمجموع</option>
                            ...
                            <form id="form_update" action="{{ route('lesson_update') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group" style="text-align:right;direction:rtl">
                                     ...
                                     <div class="form-group" style="text-align:right;direction:rtl">
                                        <label> مادة فرنسي/روسي : </label>
                                        <select name="select_lang" class="form-control w-25" id="edit_select_lang" style="display: inline-block;height: 30px;">
                                            <option value="0">فرنسي</option>
                                            <option value="1">روسي</option>
                                        </select>
                                     </div>
                                      <div class="form-group" style="text-align:right;direction:rtl">
                                        <label> مادة ديانة مسيحية/اسلامية : </label>
                                        <select name="select_religion" class="form-control w-25" id="edit_select_religion" style="display: inline-block;height: 30px;">
                                            <option value="0">اسلامية</option>
                                            <option value="1">مسيحية</option>
                                        </select>
                                     </div>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label> العلامة  العظمى للمادة </label>
                                        <input type="text" id="max_mark" name="max_mark" class="form-control a max_mark"
                                            value=""
                                            placeholder=" أدخل العلامة" maxlength="30" required>
                                    </div>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label> العلامة  الدنيا للمادة </label>
                                        <input type="text" id="min_mark" name="min_mark" class="form-control a min_mark"
                                            value=""
                                            placeholder=" أدخل العلامة" maxlength="30" required>
                                    </div>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        ...
                                        <select name="base_subject_id" id="base_subject_id" class="form-control dep"
                                            style="min-height: 36px;direction: rtl" required>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                       ...
                                       <div class="form-group" style="text-align:right" >
                                        <label for="">صورة المادة </label>
                                        <input type="file" name="image" id="" class="form-control">
                                </div>
                                    <br>
                                    <div class="form-group" style="text-align:right">
                                        <p style="font-wight:bold;color:#0069d9">تفصيلات خاصة بالجلاء المدرسي</p>
                                    </div>
                                    <div class="form-group" style="text-align:right">
                                        <label>القسم الذي تتبع له في الجلاء المدرسي </label>

                                        <select name="mark_base_subject_id" id="" class="form-control dep mark_base_subject_id"
                                            style="min-height: 36px;direction: rtl" >
                                            <option value="">اختر القسم في الجلاء </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label> هل المادة قابلة للجمع    </label>
                                         <select name="is_addable" id="" class="form-control is_addable"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل المادة قابلة للإضافة  </option>
                                    ...
                                    @endif
                                       ...
                                       <div class="form-group" style="text-align:right">
                                        <label> هل المادة  سلوك      </label>
                                         <select name="is_behavior" id="" class="form-control is_behavior"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل المادة  سلوك    </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>ترتيب الظهور في الجلاء المدرسي </label>
                                            ...
                                            <input type="number" min="0" max="16" name="certificate_order" class="form-control certificate_order"
                                                value=""
                                                placeholder="  أدخل الترتيب" maxlength="2" required>
                                    </div>
                                    <div class="form-group" style="text-align:right">
                                        <label> هل المادة مادة مشروع    </label>
                                         <select name="is_project" id="" class="form-control is_project"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden> حدد  هل المادة مادة مشروع     </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>    تدخل ضمن المجموع الأول    </label>
                                         <select name="first_total" id="" class="form-control first_total"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل تدخل المادة  في المجموع الأول   </option>
                                    ...
                                    <div class="form-group" style="text-align:right">
                                        <label>    أثر المادة على النجاح والرسوب       </label>
                                         <select name="is_neutral" id="" class="form-control is_neutral"
                                            style="min-height: 36px;direction: rtl" required>
                                            <option value="" hidden>هل تؤثر المادة  على النجاح    </option>
                ...
                <div class="modal fade deletelessonModal">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form id="form_delete" action="{{ route('lesson_delete') }}" method="POST"  autocomplete="off">

                                @csrf
                                <input type="hidden" name="lesson_id_delete" id="lesson_id_delete" required>
            ...
            const maxMarkInput = document.getElementById('max_markc');
            const minMarkInput = document.getElementById('min_markc');

            maxMarkInput.addEventListener('input', function() {
                const maxMark = parseFloat(maxMarkInput.value);
                const minMark = parseFloat(minMarkInput.value);

                if (!isNaN(maxMark) && !isNaN(minMark) && minMark > maxMark) {
                    minMarkInput.value = maxMark;
                }
            });

            minMarkInput.addEventListener('input', function() {
                const maxMark = parseFloat(maxMarkInput.value);
                const minMark = parseFloat(minMarkInput.value);

                if (!isNaN(maxMark) && !isNaN(minMark) && minMark > maxMark) {
                    alert('العلامة الدنيا لا يمكن أن تكون أكبر من العلامة العظمى');
                    minMarkInput.value = maxMark;
                }
            });
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const minMarkInput = document.getElementById('min_mark');
                        ...
                        if (value.type == "book") {
                            ...
                            $('#book_body').append(`
                                    <tr>
                                        ...
                                        <td><input type="text" name="book[${index_append}][name_ar]" value="${value.name_ar}" class="form-control"></td>
                                        <td><input type="text" name="book[${index_append}][name_en]" value="${value.name_en}" class="form-control"></td>
                                        <td class="book_td" style="max-width: 200px;display: none"><input type="file" name="book[${index_append}][file_book]" class="form-control"></td>
                                        <td class="link_td"><input type="text" name="book[${index_append}][link_book]" value="${value.value}" class="form-control"></td>
                                        <td style="max-width: 200px"><input type="file" name="book[${index_append++}][img_book]" class="form-control"></td>
                                        <td>
                                         @can('delete_lesson_book')
                                            <div class="dropdown">
                                            <a class="btn btn-danger dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">حذف</a>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                <a class="dropdown-item" href="">لا</a>
...
$(document).on('click', '.edit', function () {
    ...
    var base_subject_id=$(this).data('base_subject_id');
    var mark_base_subject_id=$(this).data('mark_base_subject_id');
    var certificate_order=$(this).data('certificate_order');
    var is_addable=$(this).data('is_addable');
    var is_behavior=$(this).data('is_behavior');
    var is_project=$(this).data('is_project');
    var first_total=$(this).data('first_total');
    var first_total=$(this).data('first_total');
    var is_neutral=$(this).data('is_neutral');
    var not_affect_and_collect=$(this).data('not_affect_and_collect');

    var max_mark =$(this).data('max_mark');
    var min_mark =$(this).data('min_mark');
    ...
    $('#name_en').val(name_en);
    $('#lesson_id').val(id);
    $('#base_subject_id').val(base_subject_id);
    $('.mark_base_subject_id').val(mark_base_subject_id);
    $('.certificate_order').val(certificate_order);
    $('.is_addable').val(is_addable);
    $('.is_behavior').val(is_behavior);
    $('.is_project').val(is_project);
    $('.first_total').val(first_total);
    $('.is_neutral').val(is_neutral);
    $('#edit_select_religion').val(religion);
    $('#edit_select_lang').val(lang);
if(not_affect_and_collect == "" && is_neutral==2 ){
         $('.is_neutral').val(4);
    }
    else{
        $('.is_neutral').val(is_neutral);
