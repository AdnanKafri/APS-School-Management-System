...
<nav class="breadcrumbs">
    ...
    <a href="{{ route('teacher_details_departments') }}" class="breadcrumbs__item ">  اقسام الادخال   </a>
...
<div class="modal fade editModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{route('teacher_details_department_fields_update')}}"  enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="create_section" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none; padding-right: 17px;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('teacher_details_department_fields_store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    ...
                    <input type="hidden" name="teacher_details_department_id"  value="{{$teacher_details_department_id}}">
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label>اسم   الحقل  </label>
        ...
        <div class="">
            <table class="table align-items-center " id="table_xx">
                ...
                <td>صورة</td>
                 ...
                 @endif
                 <td>{{$item->teacher_details_department->name}}</td>
                   
                 <td>@if($item->type==3) 
                  @if(isset($item->type_radio))
                 @foreach(json_decode($item->type_radio) as $item1)
                 ...
                 @endif</td>
                 <td>
                       @can('teacher_details_department_fields_update')
                    ...
                    @endcan
                   @can('teacher_details_department_fields_delete')
...
<div class="modal fade deletelessonModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form_delete" action="{{route('teacher_details_department_fields_delete')}}" method="POST"  autocomplete="off">
